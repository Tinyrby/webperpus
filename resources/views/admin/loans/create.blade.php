...
<div style="display: flex; align-items: center; margin-bottom: 1.5rem;">
    ...
    <h2 style="margin: 0; color: #1e293b;">Tambah Data Pinjaman</h2>
</div>
    ...
    <form action="{{ route('admin.loans.store') }}" method="POST">
        ...
        <div style="margin-bottom: 1rem;">
            ...
            <select name="member_id" class="form-control" style="width: 100%; padding: 0.6rem; border: 1px solid #cbd5e1; border-radius: 4px;" required>
                ...
                @foreach($members as $member)
                    <option value="{{ $member->id }}" {{ old('member_id') == $member->id ? 'selected' : '' }}>{{ $member->nim }} - {{ $member->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div style="display: flex; gap: 1rem; margin-bottom: 1rem;">
            <div style="flex: 1;">
                ...
                <input type="date" name="borrow_date" class="form-control" style="width: 100%; padding: 0.6rem; border: 1px solid #cbd5e1; border-radius: 4px;" value="{{ old('borrow_date') }}" required>
            </div>
            <div style="flex: 1;">
                <label style="display: block; font-weight: 500; margin-bottom: 0.5rem; color: #475569;">Tanggal Kembali</label>
                <input type="date" name="due_date" class="form-control" style="width: 100%; padding: 0.6rem; border: 1px solid #cbd5e1; border-radius: 4px;" value="{{ old('due_date') }}" required>
            </div>
        </div>
        ...
        <div>
            <button type="submit" style="background-color: var(--primary-color); color: white; border: none; padding: 0.6rem 1.5rem; border-radius: 6px; font-weight: 500; cursor: pointer;">Simpan Data</button>
        </div>
    </form>
</div>
@endsection
