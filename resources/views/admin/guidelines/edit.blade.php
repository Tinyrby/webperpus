...
<div class="admin-panel">
    <div class="admin-panel-header">
        <h3 style="font-weight: 600; color: var(--primary-dark);">Form Edit Panduan</h3>
    ...
    <form action="{{ route('admin.guidelines.update', $guideline->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="text" name="title_id" class="form-control" value="{{ old('title_id', $guideline->title_id) }}" required style="width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 4px;">
        </div>

        <div style="margin-bottom: 1.5rem;">
            <label style="display: block; font-weight: 500; margin-bottom: 0.5rem; color: #334155;">Judul Panduan (EN) <small>(Opsional)</small></label>
            <input type="text" name="title_en" class="form-control" value="{{ old('title_en', $guideline->title_en) }}" style="width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 4px;">
        </div>
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <select name="type" id="doc_type" class="form-control" style="width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 4px;">
                <option value="pdf" {{ old('type', $guideline->type) == 'pdf' ? 'selected' : '' }}>File PDF</option>
                <option value="video" {{ old('type', $guideline->type) == 'video' ? 'selected' : '' }}>Video YouTube</option>
            </select>
        </div>
        ...
        <div id="pdf_upload_section" style="margin-bottom: 1.5rem;">
            ...
            @if($guideline->file_path)
                <div style="margin-bottom: 0.5rem; padding: 0.75rem; background: #e0f2fe; border-radius: 4px; display: inline-flex; align-items: center; gap: 0.5rem;">
                    <i class="fa-solid fa-file" style="color: #0369a1;"></i>
                    <a href="{{ asset('storage/' . $guideline->file_path) }}" target="_blank" style="color: #0369a1; text-decoration: none;">Lihat File Saat Ini</a>
                </div>
            @endif
            ...
            <small style="color: #64748b;">Maksimal ukuran file: PDF (10MB), Video (50MB). Biarkan kosong jika tidak ingin mengubah file.</small>
        </div>
        ...
        <div id="video_url_section" style="margin-bottom: 1.5rem; display: none;">
            ...
            <input type="url" name="video_url" class="form-control" value="{{ old('video_url', $guideline->video_url) }}" placeholder="https://youtube.com/watch?v=..." style="width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 4px;">
        ...
        <div style="margin-bottom: 1.5rem; display: flex; gap: 1rem;">
            <div style="flex: 1;">
                ...
                <input type="number" name="order" class="form-control" value="{{ old('order', $guideline->order) }}" required style="width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 4px;">
            </div>
            <div style="flex: 1;">
                <label style="display: block; font-weight: 500; margin-bottom: 0.5rem; color: #334155;">Status Aktif</label>
                <div style="padding-top: 0.75rem;">
                    <label style="display: inline-flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $guideline->is_active) ? 'checked' : '' }} style="width: 18px; height: 18px;">
        ...
        <div style="margin-top: 2rem;">
            <button type="submit" class="btn btn-primary" style="padding: 0.75rem 2rem; font-size: 1rem;"><i class="fa-solid fa-save"></i> Perbarui Panduan</button>
        </div>
    </form>
</div>
