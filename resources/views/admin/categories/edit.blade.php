...
@section('title', 'Edit Kategori / Subjek')
...
<div style="display: flex; align-items: center; margin-bottom: 1.5rem;">
    ...
    <h2 style="margin: 0; color: #1e293b;">Edit Kategori / Subjek: {{ $category->name }}</h2>
</div>
...
<div class="card" style="max-width: 600px;">
    ...
    <form action="{{ route('admin.categories.update', $category) }}" method="POST">
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="text" name="name" class="form-control" style="width: 100%; padding: 0.6rem; border: 1px solid #cbd5e1; border-radius: 4px;" value="{{ old('name', $category->name) }}" required>
        </div>

        <div style="margin-bottom: 1.5rem;">
            <label style="display: block; font-weight: 500; margin-bottom: 0.5rem; color: #475569;">Ikon (Emoji)</label>
            <input type="text" name="icon" class="form-control" style="width: 100%; padding: 0.6rem; border: 1px solid #cbd5e1; border-radius: 4px;" value="{{ old('icon', $category->icon) }}">
            <small style="color: #64748b; margin-top: 5px; display: block;">Masukkan emoji untuk mempercantik tampilan katalog.</small>
        </div>

        <div>
            <button type="submit" style="background-color: var(--primary-color); color: white; border: none; padding: 0.6rem 1.5rem; border-radius: 6px; font-weight: 500; cursor: pointer;">Update Kategori</button>
        </div>
    </form>
</div>
@endsection
